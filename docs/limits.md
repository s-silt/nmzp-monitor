...
当前版本 0.2.3。原生沙箱和 protected-session 把 `productionReady` 设为 false。那不是这条 hook 的能力声明，hook 本身也不是沙箱。
...
| 审计不可篡改、回执持久重传 | 没做。断网时回执会丢。环满 2000 条后旧记录被挤掉。`historyCompleteness` 写成 `unknown` |
