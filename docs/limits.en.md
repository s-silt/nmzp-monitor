...
Current version 0.2.3. The native sandbox and protected-session paths set `productionReady` to false. That flag is not a certificate for the hook, and the hook is not a sandbox.
...
| Tamper-proof audit history, or durable receipt retry | Not built. A receipt during an outage can be lost. Past 2000 events, older rows are dropped. `historyCompleteness` is stored as `unknown` |
