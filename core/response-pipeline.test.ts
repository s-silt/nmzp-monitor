...
it('response → durable store → HTTPS state / LAN / map / export preserves minimal findings, coverage and transport',async()=>{
 ...
 const srv=await startServer({dataDir:join(dir,'data'),host:'127.0.0.1',port:0,coreDir:import.meta.dirname,uiDir:null});
 ...
 const gw=await startModelGateway({upstreamOrigin:`http://127.0.0.1:${addr.port}`,createUpstreamRequest:loopbackHttpFactory,onEvent:e=>{const p=storeResponseObservation(srv.store,{machineId:'fixture',sessionId:'session',agent:'grok'},e);receipts.push(p);return p;}});
 try{
  ...
  const restarted=new NmzpStore(join(dir,'data'));await restarted.load();assert.equal(restarted.listEvents()[0]?.response?.transport.outcome,'complete');
  for(const target of ['state','export']){
   ...
   const lan=await fetch(`${viewer.url}/api/v1/${target}`);assert.equal(lan.status,200);
 ...
 }finally{await gw.close();upstream.closeAllConnections();await new Promise<void>(r=>upstream.close(()=>r()));await viewer.close();await srv.close();await rm(dir,{recursive:true,force:true});}
});
